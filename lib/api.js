const API_BASE_URL = ''; // Empty = same origin (uses Next.js proxy)
 ...
 * ✅ Nu met retry logica voor 401 responses om onnodig uitloggen te voorkomen
 */
export async function apiRequest(endpoint, options = {}) {
  const { suppressErrorLog, suppressNetworkErrorLog, skipRetryOn401, ...fetchOptions } = options;
  ...
  try {
    ...
    // Verbeterde 401 handling: retry eerst voordat we uitloggen
    if (response.status === 401) {
      const currentAttempts = retryAttempts.get(endpoint) || 0;

      // Skip retry logica als expliciet gevraagd (bijv. voor auth endpoints)
      if (skipRetryOn401) {
      ...
      // Eerste poging gefaald - wacht kort en probeer opnieuw
      // Dit helpt bij race conditions waar sessie nog niet volledig is geladen
      if (currentAttempts < MAX_RETRY_ATTEMPTS) {
        retryAttempts.set(endpoint, currentAttempts + 1);

        // Exponential backoff: 500ms, 1000ms
        const delay = 500 * Math.pow(2, currentAttempts);
        console.log(`[API] 401 on ${endpoint}, retrying in ${delay}ms (attempt ${currentAttempts + 1}/${MAX_RETRY_ATTEMPTS})`);

        await new Promise(resolve => setTimeout(resolve, delay));
        return apiRequest(endpoint, options);
      }

      // Na max retries, reset counter en trigger logout
      retryAttempts.delete(endpoint);
      console.log(`[API] 401 on ${endpoint} after ${MAX_RETRY_ATTEMPTS} retries, triggering logout`);
    ...
    // Succesvolle request - reset retry counter voor dit endpoint
    retryAttempts.delete(endpoint);
 ...
 * ✅ Gebruikt nu apiRequest voor consistentie
 */
...
export async function getCurrentUser() {
  try {
    return await apiRequest('/api/users/me', {
      suppressErrorLog: true, // Silent fail for authentication checks
    });
  } catch (error) {
...
export const knowledgeBaseAPI = {
  ...
  getDocuments: (knowledgeBaseId) => apiRequest(`/api/knowledge-bases/${knowledgeBaseId}/documents`),
  getDocument: (id, workspaceId) => apiRequest(`/api/knowledge-bases/documents/${id}?workspaceId=${workspaceId}`),
  deleteDocument: (id) => apiRequest(`/api/knowledge-bases/documents/${id}`, {
    method: 'DELETE',
  }),
};
...
export const mediaAPI = {
  ...
  // ✅ Nu met API_BASE_URL voor consistentie
