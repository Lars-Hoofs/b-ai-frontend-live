...
export function CreateAgentModal({ isOpen, onClose, onSubmit, workspaceId }: CreateAgentModalProps) {
  const [name, setName] = useState('');
  const [description, setDescription] = useState('');
  const [aiModel, setAiModel] = useState('gpt-4o-mini');
  const [temperature, setTemperature] = useState(0.7);
  const [maxTokens, setMaxTokens] = useState(1000);
  const [systemPrompt, setSystemPrompt] = useState('Je bent een behulpzame AI assistent. Beantwoord vragen duidelijk en accuraat.');
  ...
  // Load workflows and knowledge bases
  useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTemperature(0.7);
      setMaxTokens(1000);
      setSystemPrompt('Je bent een behulpzame AI assistent. Beantwoord vragen duidelijk en accuraat.');
      setBlockCompetitorQuestions(false);
      setWorkflowId('');
      setKnowledgeBaseId('');
    } finally {
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title="Nieuwe Agent Aanmaken" size="xl">
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <p className="text-center text-muted-foreground mb-2">
          Configureer je AI agent met persoonlijkheid, gedrag en workflows
        </p>
        ...
        <div className="flex gap-3 pt-4">
          ...
          >
            {isSubmitting ? 'Agent aanmaken...' : 'Agent Aanmaken'}
          </button>
        </div>
      </form>
    </Modal>
